...
def _load_yaml_dict(yaml_path: Path) -> dict:
    ...
    with open(yaml_path, encoding='utf-8') as handle:
        loaded = yaml.load(handle, Loader=FremorYamlLoader)  # nosec: SafeLoader subclass with !join only
    if loaded is None:
        return {}
...
def resolve_fremor_yaml(yamlfile: str,
    ...
    combined_yaml = yaml.load(combined_yaml_text, Loader=FremorYamlLoader)  # nosec: SafeLoader subclass with !join only
